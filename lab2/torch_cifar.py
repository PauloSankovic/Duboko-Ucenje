...
def analyze(model: TorchConvolutionModel, X, Y, data_mean, data_std):
    model.eval()
    ...
    with torch.no_grad():
        for i in range(len(X)):
            logit = model.forward(torch.FloatTensor(X[[i], :, :, :]))
            prediction = np.argmax(logit)
            loss = model.get_loss(logit, torch.LongTensor(np.asarray([Y[i]])))

            predictions.append(prediction)
            if prediction == Y[i]:
                correct_classes[Y[i]] = correct_classes.get(Y[i], 0) + loss
            else:
                wrong_images[i] = loss

    sort = sorted(wrong_images.items(), key=lambda x: x[1], reverse=True)
    plot, axis = plt.subplots(5, 4)
    i = 0
    for row in axis:
        for col in row:
            index = sort[i][0]
            image = recreate_image(X[index], data_mean, data_std)
            col.imshow(image)
            col.title.set_fontsize(5)
            col.axis("off")
            col.set_title(f"{config['c'][predictions[index]]}/{config['c'][Y[index]]}")
            i += 1

    plt.savefig(f"{SAVE_DIR}/wrong-images.png")

    print("\nNajtočnije klase:")
    sort = sorted(correct_classes.items(), key=lambda x: x[1])
    print(f"{config['c'][sort[0][0]]}")
    print(f"{config['c'][sort[1][0]]}")
    print(f"{config['c'][sort[2][0]]}")
...
def main() -> None:
    ...
    analyze(model, valid_x, valid_y, data_mean, data_std)
